...
async function handleImport() {
    ...
    try {
        ...
        success.value = `Import terminé avec succès ! ${result.createdCount} ligne(s) créée(s).`
        resetFile()
    } catch (err) {
        if (err.createdCount !== undefined) {
             error.value = `${err.message} (${err.createdCount} ligne(s) créée(s) avant l'erreur).`
        } else {
